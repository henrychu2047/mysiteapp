...
export default function Page() {
  ...
  const renameCurrentProject = () => {
    ...
    if (!name) return
    if (projects.some(project => project.id !== currentProjectId && project.name === name)) { alert('Project 名稱已存在'); return }
    setProjects(current => current.map(project => project.id === currentProjectId ? { ...project, name } : project))
