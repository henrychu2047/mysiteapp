...
export default function Page() {
  ...
  const [appMode, setAppMode] = useState<'home' | 'photo' | 'memo' | 'handover' | 'reserve' | 'about'>('home')
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (savedProjects) {
        ...
        if (Array.isArray(parsed) && parsed.length) setProjects(parsed.map(project => ({ ...project, settings: { ...createProjectSettings(), ...(project.settings || {}), categories: ensureDefaultCategories(project.settings?.categories), tags: project.settings?.tags || {}, note: project.settings?.note || '', settingsOptions: project.settings?.settingsOptions || tagOptions, noteHistory: project.settings?.noteHistory || [] } })))
      }
  ...
  if (appMode === 'memo') return <SiteMemo projectId={currentProject.id} projectName={currentProject.name} onBack={() => setAppMode('home')} onOpenMachineData={() => { setHandoverView('home'); setAppMode('handover') }} onOpenMachineDataManage={() => { setHandoverView('manage'); setAppMode('handover') }} onNavigate={mode => { if (mode === 'handover') setHandoverView('manage'); setAppMode(mode); if (mode === 'photo') { setTab('home'); setActive(null) } }} />

  if (appMode === 'handover') return <Handover initialView={handoverView} projectId={currentProject.id} projectName={currentProject.name} onBack={() => setAppMode('home')} onOpenPhotoSettings={() => { setAppMode('photo'); setTab('settings'); setActive(null) }} onNavigate={mode => { setAppMode(mode); if (mode === 'photo') { setTab('home'); setActive(null) } if (mode === 'handover') { setHandoverView('manage') } }} />
  ...
  return <>
    ...
    <main className="app-shell">
      ...
      {appMode === 'home' && <section className="content home-page"><div className="section-heading"><div><p className="eyebrow">WORKSITE TOOLS</p><h2>工作首頁</h2></div></div><div className="app-card-grid"><button className="app-card app-card-photo" onClick={() => { setAppMode('photo'); setTab('home'); setActive(null) }}><Camera /><strong>拍照記錄</strong><small>{projectPhotos.length} 張相片</small></button><button className="app-card" onClick={() => setAppMode('memo')}><PenLine /><strong>Site Memo</strong><small>現場備忘及報告</small></button><button className="app-card" onClick={() => { setHandoverView('home'); setAppMode('handover') }}><ClipboardList /><strong>機房移交</strong><small>移交檢查記錄</small></button><button className="app-card" onClick={() => setAppMode('reserve')}><ShieldCheck /><strong>Permit to Work</strong><small>開發中</small></button><button className="app-card" onClick={() => setAppMode('reserve')}><BookOpen /><strong>記事簿</strong><small>開發中</small></button><button className="app-card" onClick={() => setAppMode('reserve')}><Database /><strong>資料庫</strong><small>開發中</small></button></div></section>}
      ...
      <nav className="bottom-nav main-nav"><button className={navMode === 'home' ? 'active' : ''} onClick={() => { setAppMode('home'); setTab('home'); setActive(null) }}><span><Home size={20} /></span>首頁</button><button className={navMode === 'photo' && tab === 'photos' ? 'active' : ''} onClick={() => { setAppMode('photo'); setTab('photos'); setActive(null) }}><span><Images size={20} /></span>相簿</button><button className={navMode === 'handover' ? 'active' : ''} onClick={() => { setHandoverView('manage'); setAppMode('handover') }}><span><Building2 size={20} /></span>機房資料</button><button className={navMode === 'about' ? 'active' : ''} onClick={() => setAppMode('about')}><span><Info size={20} /></span>資料</button></nav>
    </main>
