...
import { Camera, PenLine, ClipboardList, Circle, Info } from 'lucide-react'
...
export default function Page() {
  ...
  if (appMode === 'memo') return <SiteMemo projectName={currentProject.name} onBack={() => setAppMode('photo')} onNavigate={mode => { setAppMode(mode); if (mode === 'photo') { setTab('home'); setActive(null) } }} />
  ...
  return <>
    ...
    <main className="app-shell">
      ...
      <nav className="bottom-nav main-nav"><button className={appMode === 'photo' ? 'active' : ''} onClick={() => { setAppMode('photo'); setTab('home'); setActive(null) }}><span><Camera size={20} /></span>拍照記錄</button><button className={appMode === 'memo' ? 'active' : ''} onClick={() => setAppMode('memo')}><span><PenLine size={20} /></span>Site Memo</button><button className={appMode === 'handover' ? 'active' : ''} onClick={() => setAppMode('handover')}><span><ClipboardList size={20} /></span>制房移交</button><button className={appMode === 'reserve' ? 'active' : ''} onClick={() => setAppMode('reserve')}><span><Circle size={20} /></span>備用</button><button className={appMode === 'about' ? 'active' : ''} onClick={() => setAppMode('about')}><span><Info size={20} /></span>資料</button></nav>
    </main>
