...
export default function Page() {
  ...
  <Script src="https://cdnjs.cloudflare.com/ajax/libs/html2pdf.js/0.10.1/html2pdf.bundle.min.js" strategy="afterInteractive" />
    ...
    {continuousCamera && <div className="overlay dark-overlay camera-overlay"><div className="camera-sheet"><video ref={videoRef} autoPlay playsInline muted /><div className="camera-toolbar"><button className="camera-back" onClick={stopContinuousCamera} aria-label="返回上一頁">‹ 返回</button><span>連續拍攝 · {active}</span><button className={`shutter ${captureBusy ? 'is-busy' : ''}`} onClick={captureContinuousPhoto} disabled={captureBusy} aria-label="拍攝相片">{captureBusy ? '…' : '●'}</button></div>{captureMessage && <p className="capture-message" role="status">{captureMessage}</p>}{cameraError && <p className="camera-error">{cameraError}</p>}</div></div>}
