...
export default function Page() {
  ...
  const [appMode, setAppMode] = useState<'home' | 'photo' | 'memo'>('home')
  ...
  if (appMode === 'memo') return <SiteMemo onBack={() => setAppMode('home')} />
  ...
  {appMode === 'home' && <section className="content app-home"><div className="section-heading"><div><p className="eyebrow">PROJECT TOOLS</p><h2>首頁</h2></div></div><div className="app-card-grid"><button className="app-card app-card-photo" onClick={() => { setAppMode('photo'); setTab('home'); setActive(null) }}><span>▣</span><strong>拍照記錄</strong></button><button className="app-card" onClick={() => setAppMode('memo')}><span>✎</span><strong>Site memo</strong></button><button className="app-card" onClick={() => alert('制房移交功能開發中')}><span>▤</span><strong>制房移交</strong></button></div></section>}
