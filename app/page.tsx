...
export default function Page() {
  ...
  const openSharedCamera = (onCapture: (photoId: string) => void, initialCategory?: string) => {
  ...
  const sharedMediaOverlays = <>
    ...
    {cameraRequest && <ContinuousCameraModal categories={categories.map(category => category.name)} initialCategory={cameraInitialCategory} onCapture={async (file, category) => { const photo = await createProjectPhoto(file, category); cameraRequest(photo.id) }} onClose={() => setCameraRequest(null)} onOpenPhotoSettings={() => { setAppMode('photo'); setTab('home'); setActive(null) }} />}
  </>
  const importFiles = async (files: FileList | null) => {
    ...
    for (const file of Array.from(files)) {
      try {
        const photo = await createProjectPhoto(file, active)
        added.push(photo)
      } catch (error) {
        console.error('[v0] photo import failed:', error)
        failures.push(file.name)
        const reason = error instanceof Error ? error.message : '未知錯誤'
        setSaveToast(`${file.name} 處理失敗：${reason}`)
      }
    }
    if (added.length) {
      setSaveToast(failures.length ? `已加入 ${added.length} 張；${failures.length} 張失敗` : `已加入 ${added.length} 張相片`)
      setTab('photos')
    } else if (failures.length) {
      setSaveToast('相片處理失敗，請確認檔案格式及裝置儲存空間')
    }
  }
  const addProject = () => {
