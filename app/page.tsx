...
export default function Page() {
  ...
  const importFiles = async (files: FileList | null) => {
    if (!files || !selectedPhotoCategory) return
    const added: Photo[] = []
    const failures: string[] = []
    for (const file of Array.from(files)) {
      try {
        const photo = await createProjectPhoto(file, selectedPhotoCategory)
        added.push(photo)
      } catch (error) {
  ...
  return <>
    ...
    <main className="app-shell">
      ...
      {appMode === 'photo' && tab === 'home' && <section className="content"><div className="section-heading"><div><p className="eyebrow">PHOTO SETTINGS</p><h2>相片標記設定</h2></div><span className="photo-total">{currentPhotos.length} 張相片</span></div><label className="photo-category-select"><span>工程類別</span><select value={selectedPhotoCategory || ''} onChange={event => { const category = event.target.value; setActive(category); rememberCameraCategory(category) }} disabled={!categories.length}>{categories.map(category => <option key={category.name} value={category.name}>{category.name}</option>)}</select></label><div className="capture-actions"><button className="capture-button camera" disabled={!selectedPhotoCategory} onClick={() => selectedPhotoCategory && openSharedCamera(() => setSaveToast('相片已加入相簿'), selectedPhotoCategory)}><span>▣</span><div><strong>連續拍攝</strong><small>拍完可立即拍下一張</small></div></button><button className="capture-button secondary-camera" disabled={!selectedPhotoCategory} onClick={() => cameraRef.current?.click()}><span>□</span><div><strong>立即拍照</strong><small>使用 iPhone 原生相機</small></div></button><button className="capture-button album" disabled={!selectedPhotoCategory} onClick={() => albumRef.current?.click()}><span>▧</span><div><strong>選擇相簿</strong><small>可一次匯入多張</small></div></button><button className="capture-button capture-settings" disabled={!isRegistered} onClick={() => isRegistered && setShowTagDisplaySettings(true)}><span className="capture-icon"><Settings2 size={28} /></span><div><strong>設定</strong><small>{isRegistered ? '選擇要顯示的拍攝標籤' : '註冊版專有功能'}</small></div></button><input ref={cameraRef} hidden type="file" accept="image/*" capture="environment" onChange={async event => { await importFiles(event.target.files); event.currentTarget.value = '' }} /><input ref={albumRef} hidden type="file" accept="image/*" multiple onChange={async event => { await importFiles(event.target.files); event.currentTarget.value = '' }} /></div><div className="tag-panel"><div className="section-heading compact"><div><p className="eyebrow">SMART TAGS</p><h3>拍攝資訊</h3></div><span className="memory-dot">● 已記憶</span></div><div className="tag-grid">{visibleTags.map(label => { const disabled = label === '位置' && !tags['樓層']; return <button className={`tag-chip ${tags[label] ? 'chosen' : ''}`} key={label} disabled={disabled} onClick={() => { if (!disabled) setPicker(label) }}><span>{label}</span><b>{disabled ? '請先選樓層' : (tags[label] || '選擇')}</b></button> })}</div><label className="note-field"><span>文字備註</span><input value={note} onChange={event => setNote(event.target.value)} onBlur={rememberNote} onKeyDown={event => { if (event.key === 'Enter' && !event.nativeEvent.isComposing && event.keyCode !== 229) { rememberNote(); event.currentTarget.blur() } }} placeholder="輸入本次拍攝的補充說明..." /></label>{noteHistory.length > 0 && <div className="note-history"><small>最近使用</small><div>{noteHistory.map(item => <button type="button" key={item} onClick={() => setSelectedNotes(current => { const next = current.includes(item) ? current.filter(value => value !== item) : [...current, item]; setNote(next.join(' / ')); return next })} className={selectedNotes.includes(item) ? 'selected' : ''} aria-pressed={selectedNotes.includes(item)}>{item}</button>)}</div></div>}</div></section>}
