...
export default function Page() {
  ...
  return <>
    ...
    {continuousCamera && <div className="overlay dark-overlay camera-overlay"><div className="camera-sheet"><div className="camera-topline"><button className="camera-control" onClick={stopContinuousCamera} aria-label="關閉相機">×</button><span className="camera-flash" aria-label="閃光燈自動">ϟ<span>A</span></span><div className="camera-status"><i /> LIVE · {currentPhotos.length} 張</div><button className="camera-control" aria-label="更多相機設定">•••</button></div>{captureMessage && <p className="capture-message" role="status">{captureMessage}</p>}<div className="camera-frame"><video ref={videoRef} autoPlay playsInline muted /><span className="frame-corner top-left" /><span className="frame-corner top-right" /><span className="frame-corner bottom-left" /><span className="frame-corner bottom-right" /><div className="zoom-controls" aria-label="縮放倍率"><button>0.5</button><button className="selected">1×</button><button>2</button><button>5</button></div></div>{cameraError && <p className="camera-error">{cameraError}</p>}<div className="camera-toolbar"><button className="camera-control" onClick={stopContinuousCamera} aria-label="取消拍攝">×</button><div className="camera-mode">相片</div><button className={`shutter ${captureBusy ? 'is-busy' : ''}`} onClick={captureContinuousPhoto} disabled={captureBusy} aria-label="拍攝相片">{captureBusy ? '…' : ''}</button><button className="camera-control" aria-label="切換鏡頭">↻</button></div></div></div>}
