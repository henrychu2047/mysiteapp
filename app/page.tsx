...
export default function Page() {
  ...
  return <>
    ...
    <main className="app-shell">
      ...
      {appMode === 'home' && <section className="content home-page"><div className="section-heading"><div><p className="eyebrow">WORKSITE TOOLS</p></div></div><div className="app-card-grid"><button className="app-card app-card-photo" onClick={() => { setAppMode('photo'); setTab('home'); setActive(null) }}><Camera /><strong>拍照記錄</strong><small>{projectPhotos.length} 張相片</small></button><button className="app-card" onClick={() => setAppMode('memo')}><PenLine /><strong>Site Memo</strong><small>現場備忘及報告(進行中)</small></button><button className="app-card" onClick={() => { setHandoverView('home'); setAppMode('handover') }}><ClipboardList /><strong>機房移交</strong><small>移交檢查記錄</small></button><button className="app-card" onClick={() => setAppMode('notebook')}><BookOpen /><strong>記事簿</strong><small>快速記錄現場事項</small></button><button className="app-card" onClick={() => setAppMode('reserve')}><ShieldCheck /><strong>Permit to Work</strong><small>開發中</small></button><button className="app-card" onClick={() => setAppMode('reserve')}><Database /><strong>資料庫</strong><small>開發中</small></button></div></section>}
