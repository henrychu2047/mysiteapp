...
export default function Page() {
  ...
  const [appMode, setAppMode] = useState<'home' | 'photo'>('home')
  ...
  const startContinuousCamera = async () => {
    if (!navigator.mediaDevices?.getUserMedia) { setCameraError('此瀏���器不支援連續相機，請使用立即拍照'); return }
    try {
      setCameraError('')
  ...
  return <>
    ...
    <main className="app-shell">
      <header className="topbar"><button className="brand-mark" onClick={() => { setAppMode('home'); setActive(null) }} aria-label={appMode === 'photo' ? '返回首頁' : '首頁'}>{appMode === 'photo' ? '‹' : '▦'}</button><button className="project-trigger" onClick={() => setProjectPanel(true)} aria-label="選擇 Project"><strong>{currentProject.name}</strong><span>⌄</span></button>{appMode === 'photo' && <button className="icon-button" onClick={() => setNewCategory(true)} aria-label="新增類別">＋</button>}</header>
  {appMode === 'home' && <section className="content app-home"><div className="section-heading"><div><p className="eyebrow">PROJECT TOOLS</p><h2>首頁</h2></div></div><div className="app-card-grid"><button className="app-card app-card-photo" onClick={() => { setAppMode('photo'); setTab('home'); setActive(null) }}><span>▣</span><strong>拍照記錄</strong></button><button className="app-card" onClick={() => alert('Site memo 功能開發中')}><span>✎</span><strong>Site memo</strong></button><button className="app-card" onClick={() => alert('制房移交功能開發中')}><span>▤</span><strong>制房移交</strong></button></div></section>}
      ...
      {appMode === 'photo' && <nav className="bottom-nav"><button className={tab === 'home' ? 'active' : ''} onClick={() => { setTab('home'); setActive(null) }}><span>⌂</span>工程類別</button><button className={tab === 'photos' ? 'active' : ''} onClick={() => { setTab('photos'); setActive(null) }}><span>▧</span>相片集<em>{projectPhotos.length}</em></button><button className={tab === 'settings' ? 'active' : ''} onClick={() => { setTab('settings'); setActive(null) }}><span>⚙</span>設定</button></nav>}{appMode === 'home' && <nav className="bottom-nav home-bottom-nav"><button type="button"><span>◌</span>待用</button><button type="button"><span>◌</span>待用</button><button type="button"><span>⚙</span>設置</button></nav>}
    </main>
    ...
    {detail && <div className="overlay dark-overlay" onClick={() => setDetail(null)}><div className="detail-modal" onClick={e => e.stopPropagation()}><button className="detail-back" onClick={() => setDetail(null)} aria-label="返回相片集">‹ 返回</button><button className="close light" onClick={() => setDetail(null)} aria-label="關閉相片��情">×</button><img src={detail.src} alt="相片詳情" /><div className="detail-copy"><b>{detail.category}</b><p className="detail-tags">{Object.entries(detail.tags).filter(([,v]) => v).map(([k,v]) => <span key={k}>{k}: {v}</span>)}{!Object.values(detail.tags).some(Boolean) && <span>未設定標籤</span>}</p><p>{detail.note || '沒有備註'}</p><small>{new Date(detail.createdAt).toLocaleString('zh-HK')}</small></div></div></div>}
