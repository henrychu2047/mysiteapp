...
export default function Page() {
  ...
  return <>
    ...
    {showTagDisplaySettings && <div className="overlay" onClick={() => setShowTagDisplaySettings(false)}><div className="sheet" onClick={e => e.stopPropagation()}><div className="sheet-handle" /><div className="section-heading compact"><div><p className="eyebrow">SMART TAGS</p><h3>選擇顯示的 SMART TAG</h3></div><button className="close" onClick={() => setShowTagDisplaySettings(false)} aria-label="關閉">×</button></div><p className="settings-intro">選擇拍照記錄頁要顯示的標籤，設定會按目前 Project 記憶。</p><div className="settings-options tag-display-options">{SMART_TAG_KEYS.map(label => <button type="button" className={`option ${visibleTags.includes(label) ? 'chosen' : ''}`} key={label} onClick={() => setVisibleTags(current => current.includes(label) ? current.filter(item => item !== label) : [...current, label])}><span>{label}</span><b>{visibleTags.includes(label) ? '✓' : '＋'}</b></button>)}</div><button className="primary-button" onClick={() => setShowTagDisplaySettings(false)}>完成</button></div></div>}
