...
export default function Page() {
  ...
  if (appMode === 'notebook') return <><Notebook projectId={currentProject.id} projectName={currentProject.name} photoSources={projectPhotoSources} onSelectAlbumPhotos={openPhotoPicker} onOpenCamera={onCapture => openSharedCamera(photo => onCapture(photo.id))} onBack={appBack} onNavigate={mode => { setAppMode(mode); if (mode === 'photo') { setTab('photos'); setActive(null) } if (mode === 'handover') setHandoverView('settings') }} showNavigation={APP_ID === 'full'} onOpenSettings={() => { setHandoverView('settings'); setAppMode('handover') }} isRegistered={isRegistered} />{sharedMediaOverlays}{projectPickerOverlay}</>
  ...
  if (appMode === 'handover') return <><Handover initialView={handoverView} projectId={currentProject.id} projectName={currentProject.name} onProjectNameChange={name => setProjects(current => current.map(project => project.id === currentProjectId ? { ...project, name } : project))} photoSources={projectPhotoSources} onSelectAlbumPhotos={openPhotoPicker} onOpenCamera={onCapture => openSharedCamera(photo => onCapture(photo.id))} onBack={appBack} onOpenPhotoSettings={label => { setSettingsLabel(label || null); setSettingsPopupOpen(true); setAppMode('photo'); setTab('settings'); setActive(null) }} onPhotoSettingsBack={() => { setSettingsLabel(null); setSettingsPopupOpen(false); setHandoverView('settings'); setAppMode('handover') }} onSettingsBack={undefined} onStructureChange={handleStructureChange} onResponsibleEmailChange={setResponsibleEmail} isRegistered={isRegistered} onUpdateApp={updateApp} onNavigate={mode => { setSettingsPopupOpen(mode === 'backup'); setAppMode(mode); if (mode === 'photo') { setTab('photos'); setActive(null) } if (mode === 'handover') { setHandoverView('settings') } }} showNavigation={APP_ID === 'full'} onOpenSettings={openSharedSettings} />{sharedMediaOverlays}{projectPickerOverlay}</>
