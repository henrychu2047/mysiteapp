...
export default function Page() {
  ...
  useEffect(() => {
    ...
    setProjects(current => current.map(project => project.id === currentProjectId ? { ...project, settings: { ...project.settings, categories, tags, note, settingsOptions, noteHistory, visibleTags } } : project))
  ...
  const sharedMediaOverlays = <>
    ...
    {cameraRequest && <ContinuousCameraModal categories={categories.map(category => category.name)} initialCategory={cameraInitialCategory} autoStart={cameraAutoStart} onCategorySelected={rememberCameraCategory} onCapture={async (file, category) => { const photo = await createProjectPhoto(file, category); cameraRequest(photo.id) }} onClose={() => { setCameraRequest(null); setCameraAutoStart(false) }} onOpenPhotoSettings={() => { setAppMode('photo'); setTab('home'); setActive(null) }} />}
  </>
