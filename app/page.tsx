...
export default function Page() {
  ...
  return <>
    ...
    <main className="app-shell">
      <header className="topbar"><button className="brand-mark" onClick={() => { setAppMode('home'); setActive(null) }} aria-label="返回首頁">▦</button><button className="project-trigger" onClick={() => setProjectPanel(true)} aria-label="選擇 Project"><strong>{currentProject.name}</strong><span>⌄</span></button>{appMode === 'photo' && <button className="icon-button" onClick={() => setNewCategory(true)} aria-label="新增類別">＋</button>}</header>
