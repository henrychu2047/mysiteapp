...
import { Camera, PenLine, ClipboardList, Database as DatabaseIcon, BookOpen, ShieldCheck, Settings2, MapPinned } from 'lucide-react'
...
const DrawingApp = dynamic(() => import('@/components/drawings/drawing-app').then(module => module.DrawingApp), { ssr: false })
...
export default function Page() {
  ...
  const [appMode, setAppMode] = useState<'home' | 'photo' | 'memo' | 'notebook' | 'handover' | 'drawing' | 'reserve' | 'database' | 'about' | 'backup'>(INITIAL_MODE)
  ...
  const updateGoogleDriveSync = useCallback((photoId: string, googleDrive: NonNullable<Photo['googleDrive']>) => {
    ...
    if (!current) return
    const updated = { ...current, googleDrive }
    saveQueueRef.current = saveQueueRef.current.catch(() => undefined).then(() => saveStoredPhoto(updated))
    setPhotos(photos => photos.map(photo => photo.id === photoId ? updated : photo))
  }, [])
  useEffect(() => {
  ...
  const openSharedCamera = (onCapture: (photo: PhotoSource) => void, initialCategory?: string, autoStart = true) => {
  ...
  const sharedMediaOverlays = <>
    ...
    {cameraRequest && <ContinuousCameraModal categories={categories.map(category => category.name)} initialCategory={cameraInitialCategory} tags={tags} visibleTags={visibleTags} tagOptions={{ ...effectiveSettingsOptions, ...structureOptions }} note={note} noteHistory={noteHistory} selectedNotes={selectedNotes} projectName={currentProject.name} photos={projectPhotos} autoStart={cameraAutoStart} showOriginalSettingsButton={APP_ID === 'camera'} showEmbeddedCloseButton={APP_ID !== 'camera'} onCategorySelected={rememberCameraCategory} onSelectTag={(label, value) => setTags(current => ({ ...current, [label]: value }))} onNoteChange={setNote} onRememberNote={rememberNote} onToggleRecentNote={item => setSelectedNotes(current => { const next = current.includes(item) ? current.filter(value => value !== item) : [...current, item]; setNote(next.join(' / ')); return next })} onToggleVisibleTag={label => setVisibleTags(current => current.includes(label) ? current.filter(item => item !== label) : [...current, label])} onCapture={async (file, category) => { const photo = await createProjectPhoto(file, category); cameraRequest(photo) }} onOpenSettings={() => undefined} renderSettings={close => <div className="overlay camera-settings-popup-overlay"><button type="button" className="camera-settings-popup-close" onClick={() => { close(); if (APP_ID === 'camera') returnFromPhotoSettings() }} aria-label="關閉設定">×</button><Handover initialView="settings" projectId={currentProject.id} projectName={currentProject.name} photoSources={projectPhotoSources} onSelectAlbumPhotos={openPhotoPicker} onOpenCamera={onCapture => openSharedCamera(photo => onCapture(photo.id))} onBack={() => { close(); if (APP_ID === 'camera') returnFromPhotoSettings() }} onOpenPhotoSettings={label => { close(); setSettingsLabel(label || null); setAppMode('photo'); setTab('settings'); setActive(null) }} onPhotoSettingsBack={() => { close(); if (APP_ID === 'camera') returnFromPhotoSettings() }} onStructureChange={handleStructureChange} onResponsibleEmailChange={setResponsibleEmail} isRegistered={isRegistered} onUpdateApp={updateApp} onNavigate={mode => { close(); if (mode === 'backup') { setAppMode('backup'); return } setAppMode(mode) }} showToolbar={false} showNavigation={false} onOpenSettings={() => { close(); if (APP_ID === 'camera') returnFromPhotoSettings() }} /></div>} onClose={() => { setPicker(null); setCategoryPickerRequest(null); setCameraRequest(null); setCameraAutoStart(false) }} />}
  ...
  if (appMode === 'drawing') return <><DrawingApp projectId={currentProject.id} projectName={currentProject.name} categories={categories.map(category => category.name)} smartTagOptions={effectiveSettingsOptions} defaultTags={tags} photos={projectPhotos} onSelectAlbumPhotos={openPhotoPicker} onOpenCamera={(onCapture, initialCategory) => openSharedCamera(photo => onCapture(photo.id), initialCategory, true)} onRestorePhotos={restored => setPhotos(current => [...restored, ...current])} onBack={appBack} />{sharedMediaOverlays}{projectPickerOverlay}</>

  if (appMode === 'notebook') return <><Notebook projectId={currentProject.id} projectName={currentProject.name} photoSources={projectPhotoSources} onSelectAlbumPhotos={openPhotoPicker} onOpenCamera={onCapture => openSharedCamera(photo => onCapture(photo.id))} onBack={appBack} onNavigate={mode => { setAppMode(mode); if (mode === 'photo') { setTab('photos'); setActive(null) } if (mode === 'handover') setHandoverView('settings') }} showNavigation={APP_ID === 'full'} onOpenSettings={() => { setNotebookSettingsOpen(true); setHandoverView('settings'); setAppMode('handover') }} isRegistered={isRegistered} />{sharedMediaOverlays}{projectPickerOverlay}</>
  ...
  if (appMode === 'handover') return <><Handover initialView={handoverView} projectId={currentProject.id} projectName={currentProject.name} onProjectNameChange={name => setProjects(current => current.map(project => project.id === currentProjectId ? { ...project, name } : project))} photoSources={projectPhotoSources} onSelectAlbumPhotos={openPhotoPicker} onOpenCamera={onCapture => openSharedCamera(photo => onCapture(photo.id))} onBack={appBack} onOpenPhotoSettings={label => { setSettingsLabel(label || null); setSettingsPopupOpen(true); setAppMode('photo'); setTab('settings'); setActive(null) }} onPhotoSettingsBack={() => { setSettingsLabel(null); setSettingsPopupOpen(false); setHandoverView('settings'); setAppMode('handover') }} onSettingsBack={notebookSettingsOpen ? () => { setNotebookSettingsOpen(false); setAppMode('notebook') } : undefined} onStructureChange={handleStructureChange} onResponsibleEmailChange={setResponsibleEmail} isRegistered={isRegistered} onUpdateApp={updateApp} onNavigate={mode => { setSettingsPopupOpen(mode === 'backup'); setAppMode(mode); if (mode === 'photo') { setTab('photos'); setActive(null) } if (mode === 'handover') { setHandoverView('settings') } }} showNavigation={APP_ID === 'full'} onOpenSettings={openSharedSettings} popup={notebookSettingsOpen} />{sharedMediaOverlays}{projectPickerOverlay}</>
  ...
  return <>
    ...
    <main className={`app-shell app-${APP_MANIFEST.id} shell-${APP_MANIFEST.shell}`} data-app-id={APP_MANIFEST.id}>
      ...
      {appMode === 'home' && <section className="content home-page"><div className="section-heading"><div><p className="eyebrow">WORKSITE TOOLS</p></div></div><div className="app-card-grid"><button className="app-card app-card-photo" onClick={() => { const categoryNames = categories.map(category => category.name); const rememberedCategory = currentProject.settings?.lastCameraCategory; openSharedCamera(() => {}, rememberedCategory && categoryNames.includes(rememberedCategory) ? rememberedCategory : categoryNames[0], true) }}><Camera /><strong>拍照記錄</strong><small>{projectPhotos.length} 張相片</small></button><button className="app-card" onClick={() => setAppMode('memo')}><PenLine /><strong>Site Memo</strong><small>現場備忘及報告(進行中)</small></button><button className="app-card" onClick={() => { setHandoverView('home'); setAppMode('handover') }}><ClipboardList /><strong>機房移交</strong><small>移交檢查記錄</small></button><button className="app-card" onClick={() => setAppMode('notebook')}><BookOpen /><strong>記事簿</strong><small>快速記錄現場事項</small></button><button className="app-card" onClick={() => setAppMode('drawing')}><MapPinned /><strong>圖紙標記</strong><small>PDF 圖紙、問題標記及報告</small></button><button className="app-card" onClick={() => setAppMode('reserve')}><ShieldCheck /><strong>Permit to Work</strong><small>開發中</small></button><button className="app-card" onClick={() => setAppMode('database')}><DatabaseIcon /><strong>資料庫</strong><small>圖紙、Spec 及檔案管理</small></button></div></section>}
