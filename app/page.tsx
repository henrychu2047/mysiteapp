...
export default function Page() {
  ...
  useEffect(() => {
    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.register('/sw.js?v=11', { updateViaCache: 'none' }).then((registration) => {
  ...
  const captureContinuousPhoto = async () => {
    ...
    const video = videoRef.current
    if (video.readyState < 2 || !video.videoWidth || !video.videoHeight) { setCameraError('?⊿撠皞?憟踝?隢????翰?'); return }
    try {
      const canvas = document.createElement('canvas')
  ...
  return <>
    ...
    <main className="app-shell">
      ...
      {appMode === 'photo' && tab === 'home' && !active && <section className="content"><div className="quick-card-grid"><button className="quick-card" onClick={() => { setTab('photos'); setActive(null) }}><span>??/span><div><strong>?貊???/strong><small>{projectPhotos.length} 撘萇??/small></div></button><button className="quick-card" onClick={() => { setTab('settings'); setActive(null) }}><span>??/span><div><strong>閮剖?</strong><small>憿???/small></div></button></div><div className="section-heading"><div><p className="eyebrow">PROJECT ARCHIVE</p><h2>撌亦?憿</h2></div><span className="photo-total">{projectPhotos.length} 撘萇??/span></div><div className="category-grid">{categories.map(c => <button key={c.name} className="category-card" onClick={() => setActive(c.name)} onContextMenu={e => { e.preventDefault(); removeCategory(c.name) }}><span className="category-icon">{c.icon}</span><strong>{c.name}</strong><span>{projectPhotos.filter(p => p.category === c.name).length} 撘菔???/span></button>)}<button className="category-card add-card" onClick={() => setNewCategory(true)}><span className="category-icon">嚗?/span><strong>?啣?憿</strong><span>?芾?撌亦???</span></button></div><div className="hint">?瑟?憿?∠??臬?文?憿?/div></section>}
      {appMode === 'photo' && tab === 'home' && active && <section className="content"><button className="back-link" onClick={() => setActive(null)}>??餈?撌亦?憿</button><div className="section-heading"><div><p className="eyebrow">CURRENT CATEGORY</p><h2>{active}</h2></div><span className="photo-total">{currentPhotos.length} 撘?/span></div><div className="capture-actions"><button className="capture-button camera" onClick={startContinuousCamera}><span>??/span><div><strong>?????</strong><small>???舐??單?銝?撘?/small></div></button><button className="capture-button secondary-camera" onClick={() => cameraRef.current?.click()}><span>??/span><div><strong>蝡?</strong><small>雿輻 iPhone ???豢?</small></div></button><button className="capture-button album" onClick={() => albumRef.current?.click()}><span>??/span><div><strong>?豢??貊倏</strong><small>?臭?甈∪?亙?撘?/small></div></button><input ref={cameraRef} hidden type="file" accept="image/*" capture="environment" onChange={e => importFiles(e.target.files)} /><input ref={albumRef} hidden type="file" accept="image/*" multiple onChange={e => importFiles(e.target.files)} /></div><div className="tag-panel"><div className="section-heading compact"><div><p className="eyebrow">SMART TAGS</p><h3>??鞈?</h3></div><span className="memory-dot">??撌脰???/span></div><div className="tag-grid">{['璅惜', '璈', '?輸??迂', '摰', '?嗉疏?賊?', '鈭?'].map(label => <button className={`tag-chip ${tags[label] ? 'chosen' : ''}`} key={label} onClick={() => setPicker(label)}><span>{label}</span><b>{tags[label] || '?豢?'}</b></button>)}</div><label className="note-field"><span>???酉</span><input value={note} onChange={e => setNote(e.target.value)} onBlur={rememberNote} onKeyDown={e => { if (e.key === 'Enter' && !e.nativeEvent.isComposing && e.keyCode !== 229) { rememberNote(); e.currentTarget.blur() } }} placeholder="頛詨?祆活?????牧??.." /></label>{noteHistory.length > 0 && <div className="note-history"><small>?餈蝙??/small><div>{noteHistory.map(item => <button type="button" key={item} onClick={() => setSelectedNotes(current => { const next = current.includes(item) ? current.filter(value => value !== item) : [...current, item]; setNote(next.join(' / ')); return next })} className={selectedNotes.includes(item) ? 'selected' : ''} aria-pressed={selectedNotes.includes(item)}>{item}</button>)}</div></div>}</div></section>}
      {appMode === 'photo' && tab === 'settings' && <section className="content settings-page"><button className="back-link" onClick={() => { setTab('home'); setActive(null) }}>??餈?撌亦?憿</button><div className="section-heading"><div><p className="eyebrow">APP SETTINGS</p><h2>閮剖?</h2></div></div><div className="project-name-setting"><label htmlFor="project-name">Project ?迂</label><input id="project-name" value={currentProject.name} onChange={e => { const name = e.target.value; setProjects(current => current.map(project => project.id === currentProjectId ? { ...project, name } : project)) }} placeholder="頛詨 Project ?迂" /></div><p className="settings-intro">?芾??剖?蝐日??亦??賊?嚗?敺??????靘?/p><div className="local-storage-card"><strong>{saveState === 'saving' ? '甇?靽??? : saveState === 'error' ? '靽?憭望?' : '撌脖?摮?}</strong><span>{lastSavedAt ? `?敺?摮?${new Date(lastSavedAt).toLocaleString('zh-HK', { hour12: false })}` : storageStatus}</span>{storageUsage && <small>?脣?蝛粹?嚗(storageUsage.usage / 1048576).toFixed(1)} MB / {(storageUsage.quota / 1048576).toFixed(0)} MB{storageUsage.usage / storageUsage.quota > 0.8 ? '嚗餈???撱箄降?臬?遢嚗? : ''}</small>}</div>{['璅惜', '璈', '鈭?', '摰', '?嗉疏?賊?', '?輸??迂'].map(label => <div className="settings-group" key={label}><div className="settings-group-title"><strong>{label}</strong><span>{(settingsOptions[label] || []).length} ???/span></div><div className="settings-options">{(settingsOptions[label] || []).map(option => <button key={option} onClick={() => setSettingsOptions(current => ({ ...current, [label]: current[label].filter(item => item !== option) }))}>{option}<span>?</span></button>)}</div><div className="settings-add"><input value={newOption[label] || ''} onChange={e => setNewOption(current => ({ ...current, [label]: e.target.value }))} placeholder={`?啣?${label}?賊?`} /><button onClick={() => { const value = (newOption[label] || '').trim(); if (!value) return; setSettingsOptions(current => ({ ...current, [label]: [...(current[label] || []), value] })); setNewOption(current => ({ ...current, [label]: '' })) }}>?啣?</button></div></div>)}</section>}
      {appMode === 'photo' && tab === 'photos' && <section className="content"><button className="back-link" onClick={() => { setTab('home'); setActive(null) }}>??餈?撌亦?憿</button><div className="section-heading photo-heading"><div><p className="eyebrow">PHOTO ARCHIVE</p><h2>?貊???/h2></div><div className="photo-actions"><span className="photo-total">撌脤 {selected.length} 撘?/span><button className="select-all-button" onClick={() => setSelected(selected.length === projectPhotos.length ? [] : projectPhotos.map(photo => photo.id))} disabled={!projectPhotos.length}>{selected.length === projectPhotos.length && projectPhotos.length ? '???券' : '?券'}</button><button className="quick-select-button" onClick={() => { const cutoff = Date.now() - 60 * 60 * 1000; setSelected(projectPhotos.filter(photo => new Date(photo.createdAt).getTime() >= cutoff).map(photo => photo.id)) }} disabled={!projectPhotos.length}>銝撠???/button><button className="quick-select-button" onClick={() => { const cutoff = Date.now() - 24 * 60 * 60 * 1000; setSelected(projectPhotos.filter(photo => new Date(photo.createdAt).getTime() >= cutoff).map(photo => photo.id)) }} disabled={!projectPhotos.length}>銝?亙</button><div className="export-bar"><button onClick={exportExcel}>?臬 Excel</button><button onClick={exportPdf}>?臬 PDF</button></div></div></div><div className="photo-grid">{projectPhotos.map(p => <div className="photo-card" key={p.id}><button className="photo-open" onClick={() => setDetail(p)}><img src={p.src} alt={`${p.category} ${p.createdAt}`} /></button><label className="check"><input type="checkbox" checked={selected.includes(p.id)} onChange={e => setSelected(s => e.target.checked ? [...s, p.id] : s.filter(id => id !== p.id))} /><span /></label></div>)}{!projectPhotos.length && <div className="empty-state">撠?????br /><small>?脣撌亦?憿????</small></div>}</div><div id="pdf-report" className="pdf-report" aria-hidden="true"><h1>?啁?貊?閮??梯”</h1>{photos.filter(p => selected.includes(p.id)).map(p => <article key={p.id}><img src={p.src} alt="" /><div><b>{p.category}</b><p>{Object.entries(p.tags).filter(([,v]) => v).map(([k,v]) => `${k}: ${v}`).join(' / ')}</p><p>{p.note}</p><small>{new Date(p.createdAt).toLocaleString('zh-HK')}</small></div></article>)}</div></section>}
      {updateAvailable && <div className="camera-error-banner" role="status">撌脫??啁??砍?剁?隢????App???具?/div>}
      {appMode === 'about' && <section className="content info-page"><div className="section-heading"><div><p className="eyebrow">ABOUT</p><h2>鞈?</h2></div></div><div className="about-block"><h3>?甇?App</h3><p>?銝??啁撌亦??身?????極?瘀??舀?Ｙ?雿輻嚗????鞈???摮?祆?鋆蔭?蜓閬??賢??穿??閮?嚗??銝極蝔??乓?撅扎??輻??箄璅惜銝衣???Excel嚗DF ?梯”嚗ite Memo嚗??萇???A4 Site Meno嚗??嗆蝘颱漱??/p></div><div className="about-block"><h3>摰鞈??遢</h3><p>?遢?游?App ??Project??ite Memo ??輻宏鈭方???/p><div className="backup-actions"><button type="button" onClick={exportLocalBackup} disabled={backupBusy}>{backupBusy ? '甇?皞??遢?? : '?臬摰?遢'}</button><button type="button" onClick={() => backupRef.current?.click()} disabled={backupBusy}>?臬摰?遢</button><button type="button" onClick={updateApp} disabled={backupBusy}>{backupBusy ? '甇??遢??售? : '?湔 App'}</button><input ref={backupRef} hidden type="file" accept="application/zip,.zip" onChange={async e => { const file = e.target.files?.[0]; e.target.value = ''; if (!file || !confirm('?臬鞈???隞???App ??刻???衣匱蝥?')) return; await importLocalBackup(file) }} /></div></div><div className="about-block profile-block"><h3>??蝙?刻???/h3><div className="profile-card"><div className="profile-avatar" aria-hidden="true">HC</div><div className="profile-meta"><strong>Henry Chu</strong><span>Project Manager</span><span>Southa Technical Ltd</span><a href="mailto:chuwing134538@gmail.com" className="profile-email">chuwing134538@gmail.com</a></div></div></div></section>}
      <nav className="bottom-nav main-nav"><button className={appMode === 'photo' ? 'active' : ''} onClick={() => { setAppMode('photo'); setTab('home'); setActive(null) }}><span><Camera size={20} /></span>?閮?</button><button className={appMode === 'memo' ? 'active' : ''} onClick={() => setAppMode('memo')}><span><PenLine size={20} /></span>Site Memo</button><button className={appMode === 'handover' ? 'active' : ''} onClick={() => { setHandoverView('home'); setAppMode('handover') }}><span><ClipboardList size={20} /></span>?嗆蝘颱漱</button><button className={appMode === 'handover' ? 'active' : ''} onClick={() => { setHandoverView('manage'); setAppMode('handover') }}><span><Building2 size={20} /></span>璈鞈?</button><button className={appMode === 'about' ? 'active' : ''} onClick={() => setAppMode('about')}><span><Info size={20} /></span>鞈?</button></nav>
    </main>
