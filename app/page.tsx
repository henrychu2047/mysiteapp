...
export default function Page() {
  ...
  const savePhotoEdit = () => { if (!detail) return; setPhotos(current => current.map(photo => photo.id === detail.id ? detail : photo)); setPhotoEditMode(null); setPhotoDrawing([]); setSaveToast('相片註記已保存'); window.setTimeout(() => setSaveToast(''), 2500) }
  const closePhotoDetail = () => { setDetail(null); setPhotoEditMode(null); setPhotoDrawing([]) }
