...
import { Camera, PenLine, ClipboardList, Building2, Info, ChevronLeft } from 'lucide-react'
...
export default function Page() {
  ...
  const isPhotoSubpage = appMode === 'photo' && (tab !== 'home' || Boolean(active))
  const isAboutPage = appMode === 'about'
  const backToPhotoHome = () => { setTab('home'); setActive(null) }
  const backFromAbout = () => setAppMode('photo')

  return <>
    {isOffline && <div className="offline-banner" role="status">目前為離線模式，資料會儲存在本機</div>}
    <main className="app-shell">
      <header className="topbar">
        {isPhotoSubpage && <button className="topbar-back" onClick={backToPhotoHome} aria-label="返回拍照記錄首頁"><ChevronLeft size={24} /></button>}
        {isAboutPage && <button className="topbar-back" onClick={backFromAbout} aria-label="返回拍照記錄首頁"><ChevronLeft size={24} /></button>}
