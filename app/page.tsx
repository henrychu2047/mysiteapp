...
export default function Page() {
  ...
  return <>
    ...
    <main className="app-shell">
      ...
      <nav className="bottom-nav main-nav"><button className={navMode === 'photo' ? 'active' : ''} onClick={() => { setAppMode('photo'); setTab('home'); setActive(null) }}><span><Camera size={20} /></span>拍照記錄</button><button className={navMode === 'memo' ? 'active' : ''} onClick={() => setAppMode('memo')}><span><PenLine size={20} /></span>Site Memo</button><button className={navMode === 'handover' ? 'active' : ''} onClick={() => { setHandoverView('home'); setAppMode('handover') }}><span><ClipboardList size={20} /></span>制房移交</button><button className={navMode === 'handover' ? 'active' : ''} onClick={() => { setHandoverView('manage'); setAppMode('handover') }}><span><Building2 size={20} /></span>機房資料</button><button className={navMode === 'about' ? 'active' : ''} onClick={() => setAppMode('about')}><span><Info size={20} /></span>資料</button></nav>
    </main>
