...
export default function Page() {
  ...
  const applyCameraSettings = async (nextFlash: boolean, nextFocus: boolean, nextZoom: number) => {
    ...
    if (capabilities.focusMode?.includes(nextFocus ? 'continuous' : 'none')) constraints.focusMode = nextFocus ? 'continuous' : 'none'
  ...
  const toggleFlash = async () => { const next = !flashEnabled; setFlashEnabled(next); await applyCameraSettings(next, focusEnabled, zoomLevel) }
  const toggleFocus = async () => { const next = !focusEnabled; setFocusEnabled(next); await applyCameraSettings(flashEnabled, next, zoomLevel) }
  const changeZoom = async (next: number) => { setZoomLevel(next); await applyCameraSettings(flashEnabled, focusEnabled, next) }
