...
export default function Page() {
  ...
  const sharedMediaOverlays = <>
    ...
    {cameraRequest && <ContinuousCameraModal categories={categories.map(category => category.name)} initialCategory={cameraInitialCategory} tags={tags} visibleTags={visibleTags} note={note} noteHistory={noteHistory} selectedNotes={selectedNotes} autoStart={cameraAutoStart} onCategorySelected={rememberCameraCategory} onSelectCategory={(onSelect, selectedCategory) => { setCategoryPickerSelected(selectedCategory); setCategoryPickerRequest(() => onSelect); setPicker('__category__') }} onSelectTag={label => setPicker(label)} onNoteChange={setNote} onRememberNote={rememberNote} onToggleRecentNote={item => setSelectedNotes(current => { const next = current.includes(item) ? current.filter(value => value !== item) : [...current, item]; setNote(next.join(' / ')); return next })} onCapture={async (file, category) => { const photo = await createProjectPhoto(file, category); cameraRequest(photo.id) }} onClose={() => { setPicker(null); setCategoryPickerRequest(null); setCameraRequest(null); setCameraAutoStart(false) }} />}
  </>
  const addProject = () => {
