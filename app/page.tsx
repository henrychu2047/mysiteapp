...
export default function Page() {
  ...
  const [photoEditMode, setPhotoEditMode] = useState<'text' | 'marker' | 'draw' | null>(null)
  const [photoDrawing, setPhotoDrawing] = useState<Array<{ x: number; y: number }>>([])
  const photoDrawingRef = useRef<Array<{ x: number; y: number }>>([])
  ...
  useEffect(() => {
    ...
    setProjects(current => current.map(project => project.id === currentProjectId ? { ...project, settings: { categories, tags, note, settingsOptions, noteHistory, visibleTags } } : project))
  ...
  const photoPointerDown = (event: ReactPointerEvent<HTMLDivElement>) => { if (!photoEditorOpen) return; const point = photoPoint(event); if (photoEditMode === 'marker') { event.preventDefault(); if (detail) setDetail({ ...detail, annotations: [...(Array.isArray(detail.annotations) ? detail.annotations : []), { kind: 'marker', x: point.x, y: point.y }] }); return } if (photoEditMode === 'text') { event.preventDefault(); const text = prompt('輸入相片註記文字'); if (text?.trim() && detail) setDetail({ ...detail, annotations: [...(Array.isArray(detail.annotations) ? detail.annotations : []), { kind: 'text', x: point.x, y: point.y, text: text.trim() }] }); setPhotoEditMode(null); return } if (photoEditMode !== 'draw') return; event.preventDefault(); try { event.currentTarget.setPointerCapture(event.pointerId) } catch {} photoDrawingRef.current = [point]; setPhotoDrawing([point]) }
  const photoPointerMove = (event: ReactPointerEvent<HTMLDivElement>) => { if (photoEditMode !== 'draw' || !photoDrawingRef.current.length) return; event.preventDefault(); if (photoDrawingRef.current.length >= 800) return; const next = [...photoDrawingRef.current, photoPoint(event)]; photoDrawingRef.current = next; setPhotoDrawing(next) }
  const photoPointerUp = (event: ReactPointerEvent<HTMLDivElement>) => { if (photoEditMode === 'draw' && detail && photoDrawingRef.current.length > 1) { const points = photoDrawingRef.current; setDetail({ ...detail, annotations: [...(Array.isArray(detail.annotations) ? detail.annotations : []), { kind: 'draw', x: 0, y: 0, points }] }); } photoDrawingRef.current = []; setPhotoDrawing([]); try { event.currentTarget.releasePointerCapture(event.pointerId) } catch {} }
  ...
  const openPhotoEditor = () => { setPhotoEditorOpen(true); setPhotoEditMode(null); setPhotoDrawing([]) }
  const closePhotoDetail = () => { photoDrawingRef.current = []; setDetail(null); setPhotoEditorOpen(false); setPhotoEditMode(null); setPhotoDrawing([]) }
  ...
  if (appMode === 'database') return <Database projectId={currentProject.id} projectName={currentProject.name} onBack={() => setAppMode('home')} />

  if (appMode === 'memo') return <SiteMemo isRegistered={isRegistered} projectId={currentProject.id} projectName={currentProject.name} onBack={() => setAppMode('home')} onOpenMachineData={() => { setHandoverView('home'); setAppMode('handover') }} onOpenMachineDataManage={() => { setHandoverView('settings'); setAppMode('handover') }} onNavigate={mode => { if (mode === 'handover') setHandoverView('settings'); setAppMode(mode); if (mode === 'photo') { setTab('home'); setActive(null) } }} />

  if (appMode === 'handover') return <Handover initialView={handoverView} projectId={currentProject.id} projectName={currentProject.name} onBack={() => setAppMode('home')} onOpenPhotoSettings={label => { setSettingsLabel(label || null); setAppMode('photo'); setTab('settings'); setActive(null) }} onPhotoSettingsBack={() => { setSettingsLabel(null); setHandoverView('settings'); setAppMode('handover') }} onStructureChange={handleStructureChange} onResponsibleEmailChange={setResponsibleEmail} isRegistered={isRegistered} onUpdateApp={updateApp} onNavigate={mode => { setAppMode(mode); if (mode === 'photo') { setTab('home'); setActive(null) } if (mode === 'handover') { setHandoverView('manage') } }} />
