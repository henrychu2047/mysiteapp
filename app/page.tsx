...
export default function Page() {
  ...
  return <>
    ...
    <main className="app-shell">
      ...
      {appMode === 'photo' && tab === 'settings' && <section className="content settings-page"><div className="section-heading"><div><p className="eyebrow">SMART TAGS</p><h2>顯示設定</h2></div><button className="close" onClick={() => { setSettingsLabel(null); setHandoverView('settings'); setAppMode('handover'); setActive(null) }} aria-label="關閉">×</button></div><div className="tag-display-card-grid">{SMART_TAG_KEYS.map(label => <button type="button" className={`tag-display-card ${visibleTags.includes(label) ? 'chosen' : ''}`} key={label} onClick={() => setVisibleTags(current => current.includes(label) ? current.filter(item => item !== label) : [...current, label])}><strong>{label}</strong><span>{visibleTags.includes(label) ? '已顯示' : '已隱藏'}</span><b>{visibleTags.includes(label) ? '✓' : '＋'}</b></button>)}</div><button className="primary-button" onClick={() => { setSettingsLabel(null); setHandoverView('settings'); setAppMode('handover'); setActive(null) }}>完成</button></section>}
