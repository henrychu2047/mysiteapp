...
export default function Page() {
  ...
  if (appMode === 'handover') return <Handover projectId={currentProject.id} projectName={currentProject.name} onBack={() => { setAppMode('photo'); setTab('home'); setActive(null) }} onNavigate={mode => { setAppMode(mode); if (mode === 'photo') { setTab('home'); setActive(null) } }} onExportBackup={exportLocalBackup} onImportBackup={importLocalBackup} />

  return <>
    {isOffline && <div className="offline-banner" role="status">��前為離線模式，資料會儲存在本機</div>}
    <main className="app-shell">
      <header className="topbar"><div className="brand-mark" aria-hidden="true">▦</div><button className="project-trigger" onClick={() => setProjectPanel(true)} aria-label="選擇 Project"><strong>{currentProject.name}</strong><span>⌄</span></button>{appMode === 'photo' && <button className="icon-button" onClick={() => setNewCategory(true)} aria-label="新增類別">＋</button>}</header>
      ...
      {appMode === 'reserve' && <section className="content info-page"><div className="section-heading"><div><p className="eyebrow">RESERVED</p><h2>備用</h2></div></div><div className="info-empty"><span>◌</span><strong>備用功能</strong><small>此位置預留作日後擴充</small></div></section>}
      {appMode === 'about' && <section className="content info-page"><div className="section-heading"><div><p className="eyebrow">ABOUT</p><h2>資料</h2></div></div><div className="about-block"><h3>關於此 App</h3><p>這是一個為地盤工程而設的流動記錄工具，支援離線使用，所有相片與資料均保存在本機裝置。主要功能包括：拍照記錄（自動加上工程類別、樓層、機房等智能標籤並生成 Excel／PDF 報表）、Site Memo（一鍵生成 A4 公函並可 AI 行話潤色）及制房移交。</p></div><div className="about-block profile-block"><h3>開發及使用者資料</h3><div className="profile-card"><div className="profile-avatar" aria-hidden="true">HC</div><div className="profile-meta"><strong>Henry Chu</strong><span>Project Manager</span><span>Southa Technical Ltd</span></div></div></div></section>}
    </main>
    <nav className="bottom-nav main-nav"><button className={appMode === 'photo' ? 'active' : ''} onClick={() => { setAppMode('photo'); setTab('home'); setActive(null) }}><span>▣</span>拍照記錄</button><button className={appMode === 'memo' ? 'active' : ''} onClick={() => setAppMode('memo')}><span>✎</span>Site Memo</button><button className={appMode === 'handover' ? 'active' : ''} onClick={() => setAppMode('handover')}><span>▤</span>制房移交</button><button className={appMode === 'reserve' ? 'active' : ''} onClick={() => setAppMode('reserve')}><span>◌</span>備用</button><button className={appMode === 'about' ? 'active' : ''} onClick={() => setAppMode('about')}><span>ⓘ</span>資料</button></nav>
