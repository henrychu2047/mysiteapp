  ...
  return <section className="content database-page">
    ...
    <div className="database-layout">
      <aside className="database-folders">
        <strong>資料夾</strong>
        {FOLDERS.map(name => <button className={folder === name ? 'active' : ''} key={name} onClick={() => { setFolder(name); setSubfolder(''); setDrawingTower(''); setDrawingFloor(''); setDrawingPath('') }}><Folder size={18} />{name}<ChevronRight size={15} /></button>)}
      </aside>
