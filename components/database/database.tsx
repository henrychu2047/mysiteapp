  ...
  return <section className="content database-page">
    ...
    <div className="database-layout">
      ...
      <div className="database-content">
        ...
        {folder === '圖紙' && <div className="database-drawing-folders"><div className="database-subheading"><strong><FolderOpen size={17} />選擇座名</strong><span>{towers.length} 個座名</span></div>{towers.length ? <>{!drawingTower && towers.map(tower => <button key={tower.name} onClick={() => { setDrawingTower(tower.name); setDrawingFloor(''); setDrawingPath('') }}><Folder size={17} />{tower.name}<ChevronRight size={15} /></button>)}{drawingTower && <><button className="database-folder-back" onClick={() => { setDrawingTower(''); setDrawingFloor(''); setDrawingPath('') }}><ArrowLeft size={16} />返回座名</button><div className="database-subheading"><strong><FolderOpen size={17} />選擇樓數</strong><span>{selectedTower?.floors.length || 0} 個樓數</span></div>{!drawingFloor && (selectedTower?.floors || []).map(floor => <button key={floor.name} onClick={() => { setDrawingFloor(floor.name); setDrawingPath('') }}><Folder size={17} />{floor.name}<ChevronRight size={15} /></button>)}{drawingFloor && <><button className="database-folder-back" onClick={() => { setDrawingFloor(''); setDrawingPath('') }}><ArrowLeft size={16} />返回樓數</button><div className="database-subheading"><strong><FolderOpen size={17} />選擇機房</strong><span>{selectedFloor?.rooms.length || 0} 個機房</span></div>{!drawingPath && (selectedFloor?.rooms || []).map(room => { const path = `${drawingTower} / ${drawingFloor} / ${room.name}`; return <button className={drawingPath === path ? 'selected' : ''} key={room.name} onClick={() => setDrawingPath(path)}><Folder size={17} />{room.name}<ChevronRight size={15} /></button> })}</>}</>}</> : <p className="empty-state">尚未有制房資料，請先在設定建立座數、樓層及機房。</p>}</div>}
        {folder !== '圖紙' && <div className="database-drawing-folders database-custom-folders"><div className="database-subheading"><strong><FolderOpen size={17} />選擇資料夾</strong></div>{folderNames.map(name => { const fullName = `${customPrefix}${name}`; const path = `${folder} / ${fullName}`; const count = files.filter(file => file.path === path || file.path.startsWith(`${path} / `)).length; return <div className="database-folder-row" key={fullName}><button className={subfolder === fullName ? 'selected' : ''} onClick={() => setSubfolder(fullName)}><Folder size={17} /><span className="database-folder-name"><strong>{name}</strong><small>{count} 個檔案</small></span><ChevronRight size={15} /></button><button type="button" className="database-delete" onClick={() => deleteFolder(name)} aria-label={`刪除${name}資料夾`}><Trash2 size={16} /></button></div> })}</div>}
        {(folder !== '圖紙' || drawingPath) && <><div className="database-toolbar"><strong>{currentPath}</strong></div><div className="database-files">{visibleFiles.map(file => <div className="database-file" key={file.id}><button onClick={() => openFile(file)}><FileText size={24} /><span><strong>{file.name}</strong><small>{formatSize(file.size)}・{new Date(file.createdAt).toLocaleString('zh-HK', { hour12: false })}</small></span></button><button className="database-delete" onClick={() => remove(file.id)} aria-label={`刪除${file.name}`}><Trash2 size={16} /></button></div>)}{!visibleFiles.length && <p className="empty-state">此資料夾尚未有檔案。</p>}</div><div className="database-panel-actions"><button type="button" className="database-add-folder" onClick={addFolder} hidden={folder === '圖紙'}>＋新增資料夾</button><label className="database-upload"><Upload size={17} />{busy ? '上載中…' : '上載檔案'}<input hidden type="file" multiple accept="application/pdf,image/*" disabled={busy} onChange={e => { void upload(e.target.files); e.currentTarget.value = '' }} /></label></div></>}
      </div>
    </div>
