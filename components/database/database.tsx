  ...
  return <section className="content database-page">
    ...
    <div className="database-layout">
      ...
      <div className="database-content">
        ...
        {folder !== '圖紙' && <div className="database-drawing-folders database-custom-folders"><div className="database-subheading"><strong><FolderOpen size={17} />{folder}</strong></div>{folderNames.map(name => { const fullName = `${customPrefix}${name}`; const path = `${folder} / ${fullName}`; const count = files.filter(file => file.path === path || file.path.startsWith(`${path} / `)).length; return <div className="database-folder-row" key={fullName}><button className={subfolder === fullName ? 'selected' : ''} onClick={() => setSubfolder(fullName)}><Folder size={17} /><span className="database-folder-name"><strong>{name}</strong><small>{count} 個檔案</small></span><ChevronRight size={15} /></button><button type="button" className="database-delete" onClick={() => deleteFolder(name)} aria-label={`刪除${name}資料夾`}><Trash2 size={16} /></button></div> })}</div>}
