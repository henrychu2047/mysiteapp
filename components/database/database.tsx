...
export function Database({ projectId, projectName, onBack }: DatabaseProps) {
  ...
  return <>
    ...
    {viewer && <div className="database-viewer" role="dialog" aria-modal="true"><div className="database-viewer-bar"><div>{editingFile ? <input value={viewer.name} onChange={event => setViewer({ ...viewer, name: event.target.value })} /> : <strong>{viewer.name}</strong>}</div><div className="database-viewer-actions">{editingFile ? <button onClick={saveViewerEdit}>保存</button> : <button onClick={() => setEditingFile(true)}>編輯</button>}<button onClick={() => { setViewer(null); setPdfPages([]); setPdfEditMode(null) }} aria-label="關閉"><X size={21} /></button></div></div>{(viewer.type === 'application/pdf' || viewer.name.toLowerCase().endsWith('.pdf')) && <div className="database-pdf-toolbar"><strong>PDF 編輯工具</strong><button className={pdfEditMode === 'text' ? 'active' : ''} onClick={() => { setEditingFile(true); setPdfEditMode(pdfEditMode === 'text' ? null : 'text') }}>＋文字</button><button className={pdfEditMode === 'draw' ? 'active' : ''} onClick={() => { setEditingFile(true); setPdfEditMode(pdfEditMode === 'draw' ? null : 'draw') }}>✎ 手寫</button><button onClick={clearLastAnnotation} disabled={!viewer.annotations?.length}>清除最後註記</button><button className="save-annotation" onClick={saveViewerEdit}>保存註記</button><small>{pdfEditMode === 'text' ? '點擊頁面位置後輸入文字' : pdfEditMode === 'draw' ? '在頁面上拖曳手寫' : '按「＋文字」或「✎ 手寫」開始'}</small></div>}{viewer.type === 'application/pdf' || viewer.name.toLowerCase().endsWith('.pdf') ? <div className="database-pdf-pages">{pdfPages.length ? pdfPages.map((page, index) => <div className="database-pdf-page" key={index} onPointerDown={event => handlePdfPointerDown(event, index + 1)} onPointerMove={handlePdfPointerMove} onPointerUp={() => pdfEditMode === 'draw' && finishPdfDrawing(index + 1)} onPointerCancel={() => { setDrawingPoints([]); setPdfEditMode(null) }}>{<img src={page} alt={`${viewer.name} 第 ${index + 1} 頁`} />}{(viewer.annotations || []).filter(annotation => annotation.page === index + 1).map((annotation, annotationIndex) => annotation.kind === 'text' ? <span className="database-pdf-text-annotation" key={annotationIndex} style={{ left: `${annotation.x * 100}%`, top: `${annotation.y * 100}%` }}>{annotation.text}</span> : <svg className="database-pdf-draw-annotation" key={annotationIndex} viewBox="0 0 1 1" preserveAspectRatio="none"><polyline points={(annotation.points || []).map(point => `${point.x},${point.y}`).join(' ')} /></svg>)}</div>) : <iframe src={viewer.dataUrl} title={viewer.name} />}</div> : <div className="database-image-preview"><img src={viewer.dataUrl} alt={viewer.name} /></div>}</div>}
  </section>
