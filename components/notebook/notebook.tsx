...
export function Notebook({ projectId, projectName, onBack, onNavigate, photoSources, onSelectAlbumPhotos, onOpenCamera, showNavigation = true, onOpenSettings, isRegistered }: Props) {
  ...
  return <div className="app-shell notebook-app">
    ...
    <main className="notebook-body" onTouchStart={handlePullStart} onTouchMove={handlePullMove} onTouchEnd={handlePullEnd} onTouchCancel={handlePullEnd}>
      <div className="section-heading"><div><p className="eyebrow">SITE NOTEBOOK</p><h2>記事簿</h2></div><div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><button type="button" disabled={!isRegistered} onClick={() => { setQuickMode(true); openCompose() }} aria-label="開啟快速記事並使用傳送鍵保存" title={isRegistered ? '開啟快速記事' : '註冊版專有功能'} style={{ display: 'grid', placeItems: 'center', width: 42, height: 42, padding: 0, border: '1px solid var(--orange)', borderRadius: 8, background: 'var(--card)', color: 'var(--orange)', opacity: isRegistered ? 1 : .5 }}><Send size={19} aria-hidden="true" /></button><button type="button" disabled={!isRegistered} onClick={() => setQuickMode(current => !current)} aria-label="快速記事" title={isRegistered ? '快速記事' : '註冊版專有功能'} aria-pressed={quickMode} style={{ display: 'grid', placeItems: 'center', width: 42, height: 42, padding: 0, border: 0, borderRadius: 8, background: quickMode ? 'var(--blue)' : 'var(--orange)', color: 'white', opacity: isRegistered ? 1 : .5 }}><Zap size={22} aria-hidden="true" /></button>{!showNavigation && onOpenSettings && <button type="button" onClick={onOpenSettings} aria-label="設定" title="設定" style={{ display: 'grid', placeItems: 'center', width: 42, height: 42, padding: 0, border: '1px solid var(--line)', borderRadius: 8, background: 'var(--card)', color: 'var(--foreground)' }}><Settings2 size={20} aria-hidden="true" /></button>}</div></div>
