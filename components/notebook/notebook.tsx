...
export function Notebook({ projectId, projectName, onBack, onNavigate, photoSources, onSelectAlbumPhotos, onOpenCamera, showNavigation = true, onOpenSettings, isRegistered }: Props) {
  ...
  return <div className="app-shell notebook-app">
    ...
    <main className="notebook-body" onTouchStart={handlePullStart} onTouchMove={handlePullMove} onTouchEnd={handlePullEnd} onTouchCancel={handlePullCancel}>
      ...
      <div className={`notebook-modal-backdrop ${showCompose ? '' : 'notebook-modal-hidden'}`} onClick={showCompose ? closeCompose : undefined}><section className={`notebook-compose notebook-modal ${showCompose ? 'notebook-modal-visible' : ''}`} onClick={event => event.stopPropagation()}>
        <div className="notebook-modal-heading"><h2>新增記事</h2><button type="button" onClick={closeCompose}>×</button></div>
        <textarea ref={element => { composeTextareaRef.current = element; if (element && showCompose) element.focus({ preventScroll: true }) }} value={text} onChange={event => handleTextChange(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && quickMode && !event.shiftKey) { event.preventDefault(); finishQuickEntry() } else if ((event.ctrlKey || event.metaKey) && event.key === 'Enter' && !quickMode) addEntry() }} placeholder="快速記錄現場事項…" aria-label="記事內容" />
