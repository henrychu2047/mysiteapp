...
export function ContinuousCameraModal({ categories, initialCategory, tags, visibleTags, tagOptions, note, noteHistory, selectedNotes, onCapture, onClose, autoStart = false, onCategorySelected, onSelectTag, onNoteChange, onRememberNote, onToggleRecentNote, onToggleVisibleTag }: Props) {
  ...
  useEffect(() => () => { stopContinuousCamera(); if (captureFlashTimer.current !== null) window.clearTimeout(captureFlashTimer.current) }, [stopContinuousCamera])
  useEffect(() => {
  ...
  const capture = () => {
    ...
    if (captureFlashTimer.current !== null) window.clearTimeout(captureFlashTimer.current)
    captureFlashTimer.current = window.setTimeout(() => { setShowCaptureFlash(false); captureFlashTimer.current = null }, 160)
    void capturePhoto(file => onCapture(file, category))
  }
  ...
  return (
    <div className="overlay dark-overlay camera-overlay"><div className="camera-sheet">
      ...
      <div className="camera-frame"><video ref={videoRef} autoPlay playsInline muted /><span className={`camera-capture-flash ${showCaptureFlash ? 'visible' : ''}`} aria-hidden="true" /><span className="frame-corner top-left" /><span className="frame-corner top-right" /><span className="frame-corner bottom-left" /><span className="frame-corner bottom-right" /><div className="zoom-controls" aria-label="縮放倍率">{[.5, 1, 2, 5].map(level => <button key={level} onClick={() => void changeZoom(level)} className={zoomLevel === level ? 'selected' : ''}>{level === 1 ? '1×' : level}</button>)}</div></div>
      {cameraError && <p className="camera-error">{cameraError}</p>}
      <div className="camera-toolbar"><button className="camera-control" onClick={close} aria-label="取消拍攝">×</button><button className={`shutter ${captureBusy ? 'is-busy' : ''}`} onClick={capture} disabled={captureBusy} aria-label="拍攝相片">{captureBusy ? '…' : ''}</button>{autoStart ? <button className="camera-control camera-settings" onClick={() => setShowCategoryPicker(true)} aria-label="開啟相片標記設定">⚙</button> : <span>連續拍攝</span>}</div>
