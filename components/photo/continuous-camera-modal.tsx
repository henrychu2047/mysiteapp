...
type Props = {
  ...
  onSelectCategory: (onSelect: (category: string) => void, selectedCategory: string) => void
...
export function ContinuousCameraModal({ categories, initialCategory, tags, visibleTags, note, noteHistory, selectedNotes, onCapture, onClose, autoStart = false, onCategorySelected, onSelectCategory, onSelectTag, onNoteChange, onRememberNote, onToggleRecentNote, onToggleVisibleTag }: Props) {
  ...
  return (
    <div className="overlay dark-overlay camera-overlay"><div className="camera-sheet">
      ...
      {showCategoryPicker && <div className="camera-category-popup" role="dialog" aria-modal="true" aria-label="相片標記設定"><div className="sheet-handle" /><div className="section-heading compact"><div><p className="eyebrow">PHOTO SETTINGS</p><h3>相片標記設定</h3></div><button className="close" onClick={() => setShowCategoryPicker(false)} aria-label="關閉">×</button></div><div className="tag-panel"><div className="section-heading compact"><div><p className="eyebrow">SMART TAGS</p><h3>拍攝資訊</h3></div><span className="memory-dot">● 已記憶</span></div><div className="tag-grid"><button type="button" className="tag-chip category-tag-chip chosen" onClick={() => onSelectCategory(item => { setCategory(item); onCategorySelected?.(item) }, category)}><span>工程類別</span><b>{category || '選擇'}</b></button>{visibleTags.map(label => { const disabled = label === '位置' && !tags['樓層']; return <button type="button" className={`tag-chip ${tags[label] ? 'chosen' : ''}`} key={label} disabled={disabled} onClick={() => onSelectTag(label)}><span>{label}</span><b>{disabled ? '請先選樓層' : (tags[label] || '選擇')}</b></button> })}<button type="button" className={`tag-chip ${showSmartTagSettings ? 'chosen' : ''}`} onClick={() => setShowSmartTagSettings(current => !current)}><span>設定</span><b>SMART TAG 顯示</b></button></div><label className="note-field"><span>文字備註</span><input value={note} onChange={event => onNoteChange(event.target.value)} onBlur={onRememberNote} onKeyDown={event => { if (event.key === 'Enter' && !event.nativeEvent.isComposing && event.keyCode !== 229) { onRememberNote(); event.currentTarget.blur() } }} placeholder="輸入本次拍攝的補充說明..." /></label>{noteHistory.length > 0 && <div className="note-history"><small>最近使用</small><div>{noteHistory.map(item => <button type="button" key={item} onClick={() => onToggleRecentNote(item)} className={selectedNotes.includes(item) ? 'selected' : ''} aria-pressed={selectedNotes.includes(item)}>{item}</button>)}</div></div>}</div>{showSmartTagSettings && <div className="tag-panel"><div className="section-heading compact"><div><p className="eyebrow">SMART TAGS</p><h3>顯示設定</h3></div></div><div className="tag-grid">{['座數', '樓層', '位置', '系統', '設備', '事項'].map(label => <button type="button" className={`tag-chip ${visibleTags.includes(label) ? 'chosen' : ''}`} key={label} onClick={() => onToggleVisibleTag(label)}><span>{label}</span><b>{visibleTags.includes(label) ? '已顯示' : '已隱藏'}</b></button>)}</div></div>}</div>}
    </div></div>
  )
}
