...
export function ContinuousCameraModal({ categories, initialCategory, tags, visibleTags, tagOptions, note, noteHistory, selectedNotes, projectName, photos, onCapture, onClose, autoStart = false, onCategorySelected, onSelectTag, onNoteChange, onRememberNote, onToggleRecentNote, onToggleVisibleTag }: Props) {
  ...
  return (
    <div className="overlay dark-overlay camera-overlay"><div className="camera-sheet">
      ...
      {cameraError && <p className="camera-error">{cameraError}</p>}
      <div className="camera-toolbar"><button className="camera-control" onClick={close} aria-label="取消拍攝">×</button><button className={`shutter ${captureBusy ? 'is-busy' : ''}`} onClick={() => void capturePhoto(file => onCapture(file, category))} disabled={captureBusy} aria-label="拍攝相片">{captureBusy ? '…' : ''}</button>{autoStart ? <div className="camera-toolbar-actions"><button className="camera-control camera-album" onClick={() => setShowAlbum(true)} aria-label="開啟相簿"><Images size={22} strokeWidth={2.25} aria-hidden="true" /></button><button className="camera-control camera-settings" onClick={() => setShowCategoryPicker(true)} aria-label="開啟相片標記設定">⚙</button></div> : <span>連續拍攝</span>}</div>
      {showAlbum && <div className="camera-album-overlay" role="dialog" aria-modal="true" aria-label="Project 相簿"><section className="camera-album-popup"><div className="section-heading compact"><div><p className="eyebrow">PROJECT ALBUM</p><h3>相簿</h3></div><button className="close" onClick={() => setShowAlbum(false)} aria-label="關閉相簿並返回相機">×</button></div><div className="camera-album-grid">{photos.map(photo => <div className="camera-album-photo" key={photo.id}><img src={photo.src} alt={`${photo.category} ${photo.createdAt}`} /><span>{photo.category}</span></div>)}{!photos.length && <div className="empty-state">此 Project 相簿尚未有相片。</div>}</div></section></div>}
