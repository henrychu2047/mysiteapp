...
export function ContinuousCameraModal({ categories, initialCategory, tags, visibleTags, tagOptions, note, noteHistory, selectedNotes, projectName, photos, onCapture, onClose, autoStart = false, onCategorySelected, onSelectTag, onNoteChange, onRememberNote, onToggleRecentNote, onToggleVisibleTag }: Props) {
  ...
  return (
    <div className="overlay dark-overlay camera-overlay"><div className="camera-sheet">
      <div className="camera-topline camera-mode-toolbar" aria-label="相片尺寸工具列"><button type="button" className={aspectRatio === 'rectangle' ? 'selected' : ''} onClick={() => setAspectRatio('rectangle')} aria-pressed={aspectRatio === 'rectangle'}><i className="camera-mode-rectangle" aria-hidden="true" /><span>長方形</span></button><button type="button" className={aspectRatio === 'square' ? 'selected' : ''} onClick={() => setAspectRatio('square')} aria-pressed={aspectRatio === 'square'}><i className="camera-mode-square" aria-hidden="true" /><span>正方形</span></button><button type="button" disabled aria-label="功能鍵，稍後提供"><i className="camera-mode-function" aria-hidden="true">⋯</i><span>功能</span></button></div>
