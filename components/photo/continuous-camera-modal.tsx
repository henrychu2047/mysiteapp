...
type Props = {
  ...
  onClose: () => void
  onOpenSettings: () => void
  showOriginalSettingsButton?: boolean
  showEmbeddedCloseButton?: boolean
...
export function ContinuousCameraModal({ categories, initialCategory, tags, visibleTags, tagOptions, note, noteHistory, selectedNotes, projectName, photos, onCapture, onClose, onOpenSettings, showOriginalSettingsButton = true, showEmbeddedCloseButton = false, renderSettings, autoStart = false, onCategorySelected, onSelectTag, onNoteChange, onRememberNote, onToggleRecentNote, onToggleVisibleTag }: Props) {
  ...
  const [showOriginalSettings, setShowOriginalSettings] = useState(false)
  ...
  return (
    <div className="overlay dark-overlay camera-overlay"><div className="camera-sheet">
      <div className="camera-topline camera-mode-toolbar" aria-label="相片工具列"><button type="button" className={`camera-control ${aspectRatio === 'rectangle' ? 'selected' : ''}`} onClick={() => selectAspectRatio('rectangle')} aria-label="長方形拍攝" aria-pressed={aspectRatio === 'rectangle'}><i className="camera-mode-rectangle" aria-hidden="true" /></button><button type="button" className={`camera-control ${aspectRatio === 'square' ? 'selected' : ''}`} onClick={() => selectAspectRatio('square')} aria-label="正方形拍攝" aria-pressed={aspectRatio === 'square'}><i className="camera-mode-square" aria-hidden="true" /></button>{showOriginalSettingsButton && <button type="button" className="camera-control camera-settings" onClick={() => { setShowOriginalSettings(true); onOpenSettings() }} aria-label="開啟設定">⚙</button>}</div>
