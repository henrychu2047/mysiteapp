        ...
        <MemoModal title="內容與事件" className="memo-quick-generator" onClose={() => setModal(null)}>
          ...
          <div className="memo-quick-scroll">
            ...
            {templateId && <div className="memo-template-form">
              ...
              <TemplateOption label="附件段落" options={['隨函附上相關記錄以供備案。', '隨函附上相關相片以供備案。', '隨函附上相關記錄及相片以供備案。']} selected={attachmentOption ? [attachmentOption] : []} onToggle={value => setAttachmentOption(current => current === value ? '' : value)} />
    ...
    <div className="memo-modal-layer">
      <div className={`memo-modal ${className}`}>
        <div className="memo-modal-bar">
          <span className="memo-modal-bar-spacer" aria-hidden="true" />
          <strong>{title}</strong>
